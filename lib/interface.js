...
async function addEmployee(){
    ...
    let managerId;
        ...
        if(roleChoice.role == "Manager"){
            ...
            for(let i = 0; i < magObjArray.length; i++){
                managerNames.push(magObjArray[i].name);
            }; 
            const managed = await inquirer.prompt([
                {
                type: 'list',
                name: 'manager',
                message: 'Who will be manage the Employee?',
                choices: managerNames
            }]);
            managerId = indexFinder(magObjArray, 'name', managed.manager);

            sql = `INSERT INTO employee (first_name, last_name, role_id, manager_id) VALUES ("${employeeAdded.first_name}", "${employeeAdded.last_name}", (SELECT id FROM role WHERE title = "${roleChoice.role}" AND department_id = ${departmentId}), ${managerId})`;
...
async function updateEmployeeRole(){
    ...
    // try{
        ...
        if(depRole[1] == "Manager"){
            ...
            sql = `INSERT INTO manager (last_name, first_name, role_id) VALUES ("${nameCatcher[0]}", "${nameCatcher[1]}", ${rolID})`;
        } else {
