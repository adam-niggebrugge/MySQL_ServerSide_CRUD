    ...
    inquirer.prompt(
       [
        ...
        switch(choice.filter){
            ...
            case 'By Department':
                let departmentNames = [];
                departmentNames = await db.execute('SELECT name FROM department') // , (err, result) => {
                //     if(err){
                //         console.error(err);
                //         return;
                //     }
                //     // departmentNames.keys(result).forEach(result[key]);
                //     // console.log(`${departmentNames.forEach()}`);
                ...
                inquirer.prompt([
                    {
                        ...
                        sql = `SELECT * FROM (SELECT m.id AS "Employee No.", CONCAT(m.last_name, ", ", m.first_name) AS "Employee Name", r.title AS "Position", d.name AS "Department", r.salary AS "Salary", NULL AS "Manager Name" FROM manager AS m INNER JOIN role AS r ON r.id = m.role_id INNER JOIN department AS d ON d.id = r.department_id UNION ALL SELECT e.id AS "Employee No.", CONCAT(e.last_name, ", ", e.first_name) AS "Employee Name", r.title AS "Position", d.name AS "Department", r.salary AS "Salary", CONCAT(m.last_name, ", ", m.first_name) AS "Manager Name" FROM employee AS e INNER JOIN role AS r ON r.id = e.role_id INNER JOIN department AS d ON d.id = r.department_id INNER JOIN manager AS m ON m.id = e.manager_id ) AS union_result WHERE Department = '${choice.filter}'`
                    });
                break;
            case 'By Role':
                let roleNames = [];
                roleNames = await db.execute('SELECT name FROM role')
                ...
                inquirer.prompt([
                    {
                        ...
                        sql = `SELECT * FROM (SELECT m.id AS "Employee No.", CONCAT(m.last_name, ", ", m.first_name) AS "Employee Name", r.title AS "Position", d.name AS "Department", r.salary AS "Salary", NULL AS "Manager Name" FROM manager AS m INNER JOIN role AS r ON r.id = m.role_id INNER JOIN department AS d ON d.id = r.department_id UNION ALL SELECT e.id AS "Employee No.", CONCAT(e.last_name, ", ", e.first_name) AS "Employee Name", r.title AS "Position", d.name AS "Department", r.salary AS "Salary", CONCAT(m.last_name, ", ", m.first_name) AS "Manager Name" FROM employee AS e INNER JOIN role AS r ON r.id = e.role_id INNER JOIN department AS d ON d.id = r.department_id INNER JOIN manager AS m ON m.id = e.manager_id ) AS union_result WHERE Position = '${choice.filter}'`
                    });
                break;
            case 'Go Back':
                init();
