...
async function init() {
    ...
    //Iterate through result set to retain a local array for quick choices to display to user
    for(let i = 0; i < depResults.length; i++){
        let tempRowResult = '';
        //convert to string that JSON can read
        console.log(depResults[i]);

        tempRowResult = (JSON.stringify(depResults[i]));
        console.log(tempRowResult);
        console.log(JSON.parse(tempRowResult));
        //unwrap object with JSON
        depObjArray.push(JSON.parse(tempRowResult));
    };
    console.log(depObjArray);
    console.log(depObjArray[0].id);
    console.log(depObjArray[1].name);
    console.log(depObjArray[1].name === 'Receiving');

    console.log(depObjArray.find((singleObj, index) => {if(singleObj.name == 'Engineering') return index;}));
    console.log(depObjArray.splice(depObjArray.find((singleObj) => {singleObj.name == 'Engineering'}), 1));
    console.log(depObjArray);
     inquirer
        .prompt(menuChoices)
        .then((actionChoice) => {
