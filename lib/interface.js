...
async function addEmployee(){
    ...
    const [depResults, Fields] = await promisedb.query('SELECT name FROM department')
    ...
    const deptId = await promisedb.query(`SELECT id FROM department WHERE name='${depChoice.department}'`);

    const [deptRoleResults, deptFields] = await promisedb.query(`SELECT r.title FROM role AS r JOIN department AS d ON r.department_id = d.id WHERE d.id =${deptId}`);
    ...
    if(roleChoice.roleTitles == "Manager"){
        sql = `INSERT INTO manager (first_name, last_name, role_id) VALUES (${employeeAdded.first_name}, ${employeeAdded.last_name}, (SELECT id FROM role WHERE title = "Manager" AND department_id = ${deptId})`;
    } else {
        sql = `INSERT INTO employee (first_name, last_name, role_id, manager_id) VALUES (${employeeAdded.first_name}, ${employeeAdded.last_name}, (SELECT id FROM role WHERE title = "${roleChoice.roleTitles}" AND department_id = ${deptId}), (SELECT id FROM manager WHERE role_id = (SELECT id FROM role WHERE title = "Manager" AND department_id = ${deptId})`;      
    }
    init();
}
