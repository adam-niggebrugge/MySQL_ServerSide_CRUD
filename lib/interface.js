...
async function init() {
    ...
    const [empResults, Fields] = await promisedb.query('SELECT id, name FROM employee')
    objectFiller(empObjArray, empResults);
    const [mangResults, Fields] = await promisedb.query('SELECT id, name FROM manager')
    objectFiller(magObjArray, mangResults);
    const [rolResults, Fields] = await promisedb.query('SELECT id, title FROM role')
