 ...
 * returnes an index, -1 if key not applicable or find value fails
 */
function indexFinder(object, key, find){
    let index;
    switch (key){
        case 'name':
...
async function init() {
    ...
    const [rolResults, Fields3] = await promisedb.query('SELECT id, title FROM role')
...
async function addEmployee(){
    ...
    try{
        ...
        if(roleChoice.role == "Manager"){
            ...
            sql = `INSERT INTO employee (first_name, last_name, role_id, manager_id) VALUES ("${employeeAdded.first_name}", "${employeeAdded.last_name}", (SELECT id FROM role WHERE title = "${roleChoice.role}" AND department_id = ${departmentId}), (SELECT id FROM manager WHERE role_id = (SELECT TOP id FROM role WHERE title = "Manager" AND department_id = ${departmentId})))`;
