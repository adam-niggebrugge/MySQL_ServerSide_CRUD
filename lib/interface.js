        ...
        switch(getFilter.filter){
            ...
            case 'By Department':
               ...
               const [depResults, Feilds] = await promisedb.query('SELECT name FROM department')
                ...
                break;
            case 'By Role':
                const distinctRoles = getRoles('distinct');
                const roleChoice = await inquirer.prompt([
                    {
                        type: 'list',
                        name: 'role',
                        message: 'Which department?',
                        choices: distinctRoles
                    }
                ])
...
/**
 * Returns an array of the roles currently found in the database
 * @param {*} choice - string to determine whether roles should be returned based on 
 */
async function getRoles(choice){
    //Using distinct to filter out repeated roles across departments
    let roleNames = [];
    switch(choice){
        case 'distinct':
